...
func _physics_process(delta):
	# Sample from virtual Joystick
	var input_vector = Vector2.ZERO
	input_vector.x = Input.get_axis("ui_left", "ui_right")
	input_vector.y = Input.get_axis("ui_up", "ui_down")
	
	# Camera rotation
	head.rotate_y(-input_vector.x * 4 * sensitivity)
	camera.rotate_x(-input_vector.y * 4 * sensitivity)
...
#		velocity.y = JUMP_VELOCITY
	...
	# Handle sprint.
	var input_dir_sprint = Input.get_vector("left", "right", "up", "down")
	if input_dir_sprint.y <= -0.5:
		speed = SPRINT_SPEED
	else:
		speed = WALK_SPEED
